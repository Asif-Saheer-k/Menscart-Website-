...
module.exports={
    ...
    getAllofferproducts:()=>{
        return new Promise(async(resolve,reject)=>{
            let products= await db.get().collection(collection.PRODUCT_COLLECTION).find().sort({"offerpercentage":-1}).limit(10).toArray()
 ...
 return new promise(async(resolve,reject)=>{
    ...
    upadateStatus:(orderId,orderStatus)=>{
        return new promise((resolve,reject)=>{
            db.get().collection(collection.ORDER_COLLECTION).updateOne({_id:objectId(orderId)},{$set:{
                status:orderStatus,
    ...
    canceledUpdate:(oderId)=>{
        return new promise((resolve,reject)=>{
            ...
            db.get().collection(collection.ORDER_COLLECTION).updateOne({_id:objectId(oderId)},{$set:{
                canceled:true
            }}).then((response)=>{
                console.log(response);
                resolve(response)
    ...
    gettrend:()=>{
     return new promise(async(resolve,reject)=>{
     let products=await db.get().collection(collection.PRODUCT_COLLECTION).find().sort({_id:-1}).limit(10).toArray()
